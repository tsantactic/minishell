...
/*   Updated: 2024/12/24 10:27:59 by sandriam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_signal_exit_hd(char *input, int *fd, int is_last_stdin, char *delimiter);
int	checks_if_stdin(int i, t_token **token, int len);
char *write_fd_input_rd(t_token **token, char *input, int is_last_stdin, int *fd);
...
char	*initialize_expand_result(t_env **env, char *input, int *len, int *index);
...
void check_set_status_error(char **tmp_cmd, int *flags, char *command, t_env **env);
void	handle_stat_not_path(char **tmp_cmd, char *command, t_env **env);
void	execute_if_executable(t_env **env, char **tmp_cmd, char *join_path, char *command);
