...
/*   Updated: 2024/12/23 10:48:45 by tambinin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_input(t_cmd *cmd, char *input, t_env **env_list)
{
	int	tmp;
	...
	tmp = lexing_arg(input, cmd, env_list);
	if (tmp == 0)
	{
		exec_alls(cmd, env_list);
		free_tokens_all(cmd);
	}
	else if (tmp == -1)
...
void	loop_readline(t_cmd *cmd, char **env)
{
	...
	while (1)
	{
		...
		add_history(input);
		if (handle_input(cmd, input, &env_list) == 1)
			return ;
		free(input);
	}
}
